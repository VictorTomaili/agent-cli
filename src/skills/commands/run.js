...
const IMPORT_SPEC =
  /(?:from\s+|\bimport\s+)\s*['"]([^'"]+)['"]|(?:\brequire\s*\(|\bimport\s*\()\s*(?:['"]([^'"]+)['"]|([A-Za-z_$][\w$.]*))/g;
...
export async function cmdRun(args) {
  ...
  if (!isPlainSkillFile(toolPath)) {
    console.error(
      c.red(name + " tool file is a symlink/junction — refusing to run"),
    );
    process.exit(1);
  }
  try {
    ...
    if (r.output)
      console.log(r.output.endsWith("\n") ? r.output : r.output + "\n");
    if (r.ok) console.log(c.green("✓ ") + c.bold(name) + c.gray(" tool: ok"));
    else {
      console.error(
        c.red("✗ ") + c.bold(name) + c.gray(" tool: returned ok=false"),
      );
      process.exit(1);
    }
  } catch (e) {
