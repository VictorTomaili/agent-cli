...
// `agent-cli skill active` (alias: `status`) is a description-only CATALOG of ACTIVE skills;
// decides per skill: functional → `agent-cli skill cat`, context-altering → propose. No flag
...
This machine uses the \`agent-cli skill\` command to manage skills (instruction /
...
Usage:
- \`agent-cli skill list\` — skills installed + active in the current project (with triggers)
- \`agent-cli skill show <name>\` — skill summary (path, triggers, version)
- \`agent-cli skill cat <name>\` — load skill content into context
- \`agent-cli skill default <name>\` — mark a skill as a global default (active + auto-load)
