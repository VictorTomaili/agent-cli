...
export function cmdUpdate(args) {
	...
	console.log(
		c.bold(`Updating ${targets.length} skill(s)…`) +
			c.gray("  (re-fetch from source)"),
	);
	console.log();
	...
	console.log(
		c.gray(
			`  ${counts.updated} updated · ${counts.current} up to date · ${counts.failed} failed · ${counts.nosource} no source`,
		),
	);
...
export function updateOne(entry) {
	...
	if (!fs.existsSync(sourceFile)) {
		console.log(
			c.gray("  · " + pad(name)) +
				c.gray("source unknown — reinstall: skill install <source>"),
		);
		return "nosource";
	}
	...
	try {
		...
		console.log(
			c.red("  ✗ " + pad(name)) + c.red((e.message || "").split("\n")[0]),
		);
		return "failed";
	}
...
function resolveFetched(fetchedDir, name) {
	const entries = fs
		.readdirSync(fetchedDir, { withFileTypes: true })
