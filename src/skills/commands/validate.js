...
export function validateSkill(content, { fileName: _fileName = 'SKILL.md', dirName } = {}) {
  ...
  if (name) {
    ...
    if (n.length > 64) errors.push(`skill name exceeds the 64-character limit (${n.length} chars) — Agent Skills spec`)
    if (n !== n.toLowerCase()) errors.push(`skill name must be lowercase — got "${name}"`)
    ...
    if (!/^[\p{Ll}\p{Nd}-]*$/u.test(n)) errors.push('skill name contains invalid characters — only lowercase letters, digits, and hyphens are allowed')
    if (dirName && dirName.normalize('NFKC') !== n) errors.push(`directory name "${dirName}" must match skill name "${n}" (Agent Skills spec)`)
  }

  // --- spec: description (required, 1–1024) ---
  const desc = stringField(data.description)
  if (!desc) errors.push('frontmatter `description` is required — 1-1024 chars saying what the skill does and when to use it')
  else if (desc.length > 1024) errors.push(`description exceeds the 1024-character limit (${desc.length} chars)`)

  // --- spec: optional typed fields ---
  if (data.compatibility !== undefined) {
    if (typeof data.compatibility !== 'string') errors.push('frontmatter `compatibility` must be a string')
    else if (data.compatibility.length > 500) errors.push(`compatibility exceeds the 500-character limit (${data.compatibility.length} chars)`)
  }
  if (data.license !== undefined && typeof data.license !== 'string') errors.push('frontmatter `license` must be a string')
  if (data['allowed-tools'] !== undefined && typeof data['allowed-tools'] !== 'string') errors.push('frontmatter `allowed-tools` must be a space-separated string of tool names')
  if (data.metadata !== undefined) {
    const md = getMetadata(data)
    if (md) for (const [k, v] of Object.entries(md)) {
      if (typeof v !== 'string') warnings.push(`metadata value for "${k}" is not a string — the spec defines metadata as string → string`)
    } else errors.push('frontmatter `metadata` must be a mapping (the spec defines it as string → string)')
  }

  // --- spec: closed allowlist (+ our documented extensions) ---
  const allowed = new Set([...SPEC_FIELDS, ...AGENT_CLI_EXT_FIELDS])
  const extra = Object.keys(data).filter((k) => !allowed.has(k)).sort()
  if (extra.length) {
    errors.push(`unexpected fields in frontmatter: ${extra.join(', ')} — only ${SPEC_FIELDS.join(', ')} (Agent Skills spec) plus ${AGENT_CLI_EXT_FIELDS.join(', ')} (agent-cli extension) are allowed`)
  }

  // --- agent-cli extensions: type checks + portability warnings ---
  const vRaw = data.version
  if (vRaw !== undefined && typeof vRaw !== 'string' && typeof vRaw !== 'number') {
    errors.push('frontmatter `version` must be a string or number')
  }
  if (vRaw !== undefined) {
    warnings.push(`top-level \`version\` is an agent-cli extension — for portability move it under metadata: { "${EXT_NS}.version": "1.0.0" }`)
  }
  const extMeta = getMetadata(data)
  if (extMeta) {
    const mv = extMeta[`${EXT_NS}.version`]
    if (mv !== undefined && typeof mv !== 'string') errors.push(`metadata "${EXT_NS}.version" must be a string`)
  }
  if (data.triggers !== undefined) {
    ...
    else warnings.push(`top-level \`triggers\` is an agent-cli extension — for portability move it under metadata: { "${EXT_NS}.triggers": "a, b" }`)
  }
...
export function cmdValidate(args) {
  ...
  const v = validateSkill(content, { dirName: path.basename(path.dirname(res.path)) })
