	...
	switch (cmd) {
		...
		case "ui":
			if (!isInteractive()) {
				console.error(
					c.red(
						"'skill manager' is interactive — run it in a terminal, or use 'skill list'.",
					),
				);
				process.exit(1);
			}
		...
		case "browse":
			if (!isInteractive()) {
				console.error(
					c.red(
						"'skill search' is interactive — run it in a terminal, or use 'skill install <source>'.",
					),
				);
				process.exit(1);
			}
